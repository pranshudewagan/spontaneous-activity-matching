-- Phase 4e fix: manual mode now respects capacity at swipe time.
-- Previously, manual always inserted as 'interested' regardless of capacity,
-- which contradicted the spec ("every activity has a waitlist; at capacity,
-- new right-swipes go to the waitlist in order").
-- Fix: if the activity is full, manual swipes land as 'waitlisted' too.
